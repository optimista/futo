...
import { Button, Dialog, DialogActions, DialogContent, DialogTitle, NoSsr, TextField } from '@mui/material'
  ...
  return (
    <>
      <PostCardLayout avatar={ auth.isLoggedIn && !auth.isAnonymous && <ProfileAvatar /> } sx={{ mb: 2 }} title={
        <NoSsr><TextField inputProps={{ disabled: true }} margin="normal" onMouseDown={handleMouseDown} placeholder={l("prompt", PROMPT, locale)} sx={{ flex: "auto", my: 2, '& > .MuiInput-root > input': { cursor: "pointer" } }} /></NoSsr>}
      />
